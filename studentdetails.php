<?php require_once("partials/_cache.php");$delete=false;session_start();if(!isset($_SESSION['loggedin'])||$_SESSION['loggedin']!=true){header("location: /");exit;} ?>
<!doctypehtml>
    <html lang="en">
    ...
    <body>
        ...
        <aside><?php include 'partials/_sidebar.php'; ?></aside><?php require 'partials/_function.php'; ?><main
            ...
            <?php $student_id=$_GET['id'];function marks_calc_com($sql,$conn,$student_id){$portion_avg="SELECT AVG(total) FROM mid_committee WHERE st_te_assigned_id IN ( SELECT assigned_id FROM mid_committee_assigned WHERE assigned_s_id = '$student_id')";mysqli_query($conn,$sql);$result_avg=mysqli_query($conn,$portion_avg);$row_avg=mysqli_fetch_assoc($result_avg);$portion=$row_avg['AVG(total)']/100*10;$marks_portion="UPDATE `total_marks` SET `tm_mid_com` = '$portion'  WHERE `tm_student_id` = '$student_id'";mysqli_query($conn,$marks_portion);}function marks_calc_ext($sql,$conn,$student_id,$portion){$marks_portion="UPDATE `total_marks` SET `tm_mid_ext` = '$portion'  WHERE `tm_student_id` = '$student_id'";mysqli_query($conn,$sql);mysqli_query($conn,$marks_portion);}if(isset($_GET['sdelete'])){$assigned_id_1=e($_GET['sdelete']);echo $assigned_id_1;$sql="DELETE FROM `mid_supervisor_assigned` WHERE `assigned_id` = '$assigned_id_1'";$sql1="UPDATE `total_marks` SET `tm_mid_sup` = '0'  WHERE `tm_student_id` = '$student_id'";mysqli_query($conn,$sql);mysqli_query($conn,$sql1);$delete=true;}elseif(isset($_GET['edelete'])){$assigned_id_1=e($_GET['edelete']);echo $assigned_id_1;$sql="DELETE FROM `mid_external_assigned` WHERE `assigned_id` = '$assigned_id_1'";$sql1="UPDATE `total_marks` SET `tm_mid_ext` = '0'  WHERE `tm_student_id` = '$student_id'";mysqli_query($conn,$sql);mysqli_query($conn,$sql1);$delete=true;}elseif(isset($_GET['cdelete'])){$assigned_id_1=e($_GET['cdelete']);echo $assigned_id_1;$sql="DELETE FROM `mid_committee_assigned` WHERE `assigned_id` = '$assigned_id_1'";mysqli_query($conn,$sql);$portion_avg="SELECT AVG(total) FROM mid_committee WHERE st_te_assigned_id IN ( SELECT assigned_id FROM mid_committee_assigned WHERE assigned_s_id = '$student_id')";$result_avg=mysqli_query($conn,$portion_avg);$row_avg=mysqli_fetch_assoc($result_avg);$portion=$row_avg['AVG(total)']/100*10;$marks_portion="UPDATE `total_marks` SET `tm_mid_com` = '$portion'  WHERE `tm_student_id` = '$student_id'";mysqli_query($conn,$marks_portion);$delete=true;}if($_SERVER['REQUEST_METHOD']=='POST'){if(!empty($_POST['committee'])){foreach($_POST['committee']as $selected){$sql="INSERT INTO `mid_committee_assigned` (`assigned_s_id`, `assigned_teacher_id`) SELECT student_id, (SELECT teacher_id FROM teacher WHERE teacher_id = '$selected') FROM students WHERE student_id='$student_id'";mysqli_query($conn,$sql);}}if(isset($_POST['supervisor'])){$supervisor_id=$_POST['supervisor'];$sql="INSERT INTO `mid_supervisor_assigned` (`assigned_s_id`, `assigned_teacher_id`) SELECT student_id, (SELECT teacher_id FROM teacher WHERE teacher_id = '$supervisor_id') FROM students WHERE student_id='$student_id'";mysqli_query($conn,$sql);}if(isset($_POST['external'])){$external_id=$_POST['external'];$sql="INSERT INTO `mid_external_assigned` (`assigned_s_id`, `assigned_ext_id`) SELECT student_id, (SELECT external_id FROM ext_teacher WHERE external_id = '$external_id') FROM students WHERE student_id='$student_id'";mysqli_query($conn,$sql);}if(isset($_POST['supervisor_assigned_id'])){$identity=$_POST['supervisor_assigned_id'];$action=substr($identity,0,2);$sno=substr($identity,2);$par1=mysqli_real_escape_string($conn,$_POST['regularity']);$par2=mysqli_real_escape_string($conn,$_POST['completeness_degree']);$par3=mysqli_real_escape_string($conn,$_POST['understanding_thesis']);$par4=mysqli_real_escape_string($conn,$_POST['effort']);$par5=mysqli_real_escape_string($conn,$_POST['organization']);$sum=$par1+$par2+$par3+$par4+$par5;$portion=$sum/100*20;if($action=="sa"){$sql="INSERT INTO `mid_supervisor` (`st_te_assigned_id`, `par1`, `par2`, `par3`, `par4`, `par5`, `total`) VALUES ('$sno', '$par1', '$par2', '$par3', '$par4', '$par5', '$sum')";}elseif($action=="se"){$sql="UPDATE `mid_supervisor` SET `par1` = '$par1', `par2` = '$par2', `par3` = '$par3', `par4` = '$par4', `par5` = '$par5', `total` = '$sum' WHERE `st_te_assigned_id` = '$sno'";}$marks_portion="UPDATE `total_marks` SET `tm_mid_sup` = '$portion'  WHERE `tm_student_id` = '$student_id'";mysqli_query($conn,$sql);mysqli_query($conn,$marks_portion);}if(isset($_POST['assigned_id'])){$identity=$_POST['assigned_id'];$action=substr($identity,0,2);$sno=substr($identity,2);$par1=mysqli_real_escape_string($conn,$_POST['quality_of_presentation']);$par2=mysqli_real_escape_string($conn,$_POST['problem_identification']);$par3=mysqli_real_escape_string($conn,$_POST['methodology']);$par4=mysqli_real_escape_string($conn,$_POST['literature_review']);$par5=mysqli_real_escape_string($conn,$_POST['understanding']);$par6=mysqli_real_escape_string($conn,$_POST['answers']);$par7=mysqli_real_escape_string($conn,$_POST['completeness']);$par8=mysqli_real_escape_string($conn,$_POST['planning']);$sum=$par1+$par2+$par3+$par4+$par5+$par6+$par7+$par8;$portion=$sum/100*10;if($action=="ea"){$sql="INSERT INTO `mid_external` (`st_te_assigned_id`, `par1`, `par2`, `par3`, `par4`, `par5`, `par6`, `par7`, `par8`, `total`) VALUES ('$sno', '$par1', '$par2', '$par3', '$par4', '$par5', '$par6', '$par7', '$par8', '$sum')";marks_calc_ext($sql,$conn,$student_id,$portion);}elseif($action=="ee"){$sql="UPDATE `mid_external` SET `par1` = '$par1', `par2` = '$par2', `par3` = '$par3', `par4` = '$par4', `par5` = '$par5', `par6` = '$par6', `par7` = '$par7', `par8` = '$par8', `total` = '$sum' WHERE `st_te_assigned_id` = '$sno'";marks_calc_ext($sql,$conn,$student_id,$portion);}elseif($action=="ca"){$sql="INSERT INTO `mid_committee` (`st_te_assigned_id`, `par1`, `par2`, `par3`, `par4`, `par5`, `par6`, `par7`, `par8`, `total`) VALUES ('$sno', '$par1', '$par2', '$par3', '$par4', '$par5', '$par6', '$par7', '$par8', '$sum')";marks_calc_com($sql,$conn,$student_id);}elseif($action=="ce"){$sql="UPDATE `mid_committee` SET `par1` = '$par1', `par2` = '$par2', `par3` = '$par3', `par4` = '$par4', `par5` = '$par5', `par6` = '$par6', `par7` = '$par7', `par8` = '$par8', `total` = '$sum' WHERE `st_te_assigned_id` = '$sno'";marks_calc_com($sql,$conn,$student_id);}}} ?>
                    ...
                    <div class="mb-4 row">
                        <div class="text-muted d-flex justify-content-end col-md-7">
                            <div class="btn-group" aria-label="Basic example" role="group"><a
                                    href="studentdetails.php?id=<?php echo $student_id; ?>" class="btn btn-primary"
                                    role="button" active>MidTerm</a> <a
                                    href="finalstudentdetails.php?id=<?php echo $student_id; ?>"
                                    class="border border-1 btn border-primary btn-outline-primary"
                                    role="button">FinalTerm</a></div>
                        </div>
                    </div>
                    <div class="row">
