<?php if(isset($_GET['id'])){$id=$_GET['id'];$sql="SELECT * FROM `students` WHERE `student_id` = $id";$result=mysqli_query($conn,$sql);if(mysqli_num_rows($result)==0){echo '
    ...
    ';exit();}}else{exit();} ?><div class="card mb-4 mt-1"><div class="card-header pb-2 py-3"><strong>Student Information</strong></div><div class="card-body"><div class="profile-pic"><?php $row=mysqli_fetch_assoc($result);if($row['student_gender']=='Male'){echo '
                ...
                ';} ?></div><div class="container-fluid mt-4"><?php echo '
                <div class="row">
                ...
                <div class="row">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#marksDetailsModal">
                        <i class="fa fa-info-circle fa-lg" style="color: #ffffff !important;"></i>
                ...
                </div>'; ?></div></div></div><div class="fade modal"aria-hidden="true"aria-labelledby="marksDetailsModalLabel"id="marksDetailsModal"><div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header"><h5 class="modal-title"id="marksDetailsModalLabel"><strong>Marks Details<?php echo ' ('.e($row['student_fname']).' '.e($row['student_lname']).')' ?></strong></h5><button class="btn-close"data-bs-dismiss="modal"type="button"aria-label="Close"></button></div><div class="modal-body"><div class="table-responsive"><table class="table table-hover table-striped"id="marks-list"style="width:100%"><colgroup><col span="1"style="width:30%"><col span="1"style="width:25%"><col span="1"style="width:25%"><col span="1"style="width:20%"></colgroup><thead><tr><th></th><th>Mid</th><th>Final</th><th>Total</th></tr></thead><tbody><?php $std_id=$_GET['id'];$sql="SELECT * FROM `total_marks` WHERE `tm_student_id`='$std_id'";$result=mysqli_query($conn,$sql);$student_marks=mysqli_fetch_assoc($result);echo '
                            <tr>
                                <th>Supervisor</th>
                            ...
                            </tfoot>'; ?></table></div></div><div class="modal-footer"><button class="btn btn-secondary"data-bs-dismiss="modal"type="button">Close</button></div></div></div></div>