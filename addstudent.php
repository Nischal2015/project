...
<body>
    ...
    <main class="mt-1 p-2" style="min-height:800px">
        <div class="container-fluid page-header">
            <?php $showAlert=false;if($_SERVER['REQUEST_METHOD']=='POST'){$fname=mysqli_real_escape_string($conn,$_POST['fname']);$lname=mysqli_real_escape_string($conn,$_POST['lname']);$roll=mysqli_real_escape_string($conn,$_POST['roll']);$dep=mysqli_real_escape_string($conn,$_POST['department']);$gender=mysqli_real_escape_string($conn,$_POST['gender']);$regdate=mysqli_real_escape_string($conn,$_POST['regdate']);$year=mysqli_real_escape_string($conn,$_POST['year']);$thesis=mysqli_real_escape_string($conn,$_POST['thesis']);echo $fname."<br>";echo $lname."<br>";echo $roll."<br>";echo $dep."<br>";echo $gender."<br>";echo $regdate."<br>";echo $year."<br>";echo $thesis."<br>";$sql="INSERT INTO `students` (`student_fname`, `student_lname`, `student_roll`, `student_dep`, `student_gender`, `student_regdate`, `student_year`, `student_thesis`) VALUES ('$fname', '$lname', '$roll', '$dep', '$gender', '$regdate', '$year', '$thesis')";$result=mysqli_query($conn,$sql);echo var_dump($result);$getid="SELECT `student_id` FROM `students` WHERE `student_roll` = '$roll'";$result1=mysqli_query($conn,$getid);$row1=mysqli_fetch_assoc($result1);$student_id=$row1['student_id'];$sql_marks="INSERT INTO `total_marks` (`tm_student_id`, `tm_mid_sup`, `tm_mid_com`, `tm_mid_ext`, `tm_final_sup`, `tm_final_com`, `tm_final_ext`) VALUES ('$student_id', '0', '0', '0', '0', '0', '0')";mysqli_query($conn,$sql_marks);$showAlert=true;}if($showAlert){echo '            
